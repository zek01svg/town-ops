...
// ponytail: only the two keys `src/env.ts` actually reads (the Resident app
// has no Google Maps key); a key whose env var is unset is omitted rather
// than injected as `""`, so the client's own
// `?? import.meta.env.X ?? <default>` fallback chain still runs for it.

...
const COLUMNS = [
  ...
  { id: "dispatched",  label: "Dispatched", statuses: ["assigned", "dispatched", "in_progress"] },
...
export function OfficerDashboard() {
  ...
  const columns = [
    ...
    columnHelper.accessor("status", {
      header: "Status",
      cell: (info) => (
        <Badge className="rounded-none bg-muted text-foreground border-border uppercase text-[10px]">{info.getValue()}</Badge>
      ),
    }),
    columnHelper.display({
      ...
      cell: (info) => (
        <Button
          ...
          onClick={() => { setSelectedCaseId(info.row.original.id); setIsAuditOpen(true); }}
        >
          View Audit
        </Button>
      ),
  ...
  const resolvedCases = cases.filter((c) => ["completed", "cancelled"].includes(c.status));

  return (
    <div className="flex flex-col gap-6">
      {/* Audit Sheet */}
      <Sheet open={isAuditOpen} onOpenChange={setIsAuditOpen}>
        <SheetContent side="right" className="bg-popover shadow-2xl text-foreground border-border rounded-none sm:max-w-md w-full">
          <SheetHeader>
      ...
      <div className="flex items-center justify-between">
        ...
        <Sheet open={isSheetOpen} onOpenChange={setIsSheetOpen}>
          ...
          <SheetContent side="right" className="bg-popover border-border rounded-none sm:max-w-md w-full overflow-y-auto">
            <SheetHeader>
              ...
              <SheetDescription className="text-muted-foreground font-sans">Fill out the incident details payload carefully.</SheetDescription>
            </SheetHeader>
      ...
      <div className="grid gap-4 grid-cols-2 lg:grid-cols-4">
        {[
          { t: "Active Cases",  v: isLoading ? "—" : String(activeCases.length),  s: "Open reports",          ic: "text-foreground" },
          { t: "SLA Breached",  v: isLoading ? "—" : String(breachedCases.length), s: "Action required",       ic: "text-destructive", border: "border-t-4 border-t-destructive" },
          { t: "Total Cases",   v: isLoading ? "—" : String(cases.length),          s: "All time",              ic: "text-amber-500",  border: "border-t-4 border-t-amber-500" },
          { t: "Resolved",      v: isLoading ? "—" : String(resolvedCases.length),  s: "Completed or cancelled", ic: "text-green-500", border: "border-t-4 border-t-green-500" },
        ].map((item) => (
          <Card key={item.t} className={`bg-surface-container border border-border rounded-none ${item.border ?? ""}`}>
                      ...
                      <div className="flex flex-col gap-2 p-3 flex-1">
                        {cards.length === 0 ? (
                          ...
                          <CaseKanbanCard
                            ...
                            onClick={() => { setSelectedCaseId(c.id); setIsAuditOpen(true); }}
                          />
                        ))}
                      </div>
                    </div>
                ...
                <Table>
                  ...
                  <TableBody>
                    {table.getRowModel().rows.length === 0 ? (
                      <TableRow>
                        <TableCell colSpan={columns.length} className="text-center text-muted-foreground py-8 text-sm">No cases found.</TableCell>
                      </TableRow>

      ...
      {caseData && (
        <div className="flex flex-col gap-2 border border-border p-4 bg-card">
          ...
          <div className="flex justify-between items-center">
            ...
            <Badge className="rounded-none text-[10px] uppercase bg-muted text-foreground border-border">{caseData.category}</Badge>
          </div>
          <div className="flex justify-between items-center">
            <span className="text-[10px] font-label uppercase tracking-widest text-muted-foreground">Priority</span>
            <Badge variant={caseData.priority === "high" || caseData.priority === "emergency" ? "destructive" : "outline"} className="rounded-none text-[10px] uppercase">{caseData.priority}</Badge>
          </div>
          <div className="flex justify-between items-center">
            <span className="text-[10px] font-label uppercase tracking-widest text-muted-foreground">Status</span>
            <Badge className="rounded-none text-[10px] uppercase bg-muted text-foreground border-border">{caseData.status}</Badge>
          </div>
          {caseData.address && (
        ...
        <div className="flex flex-col gap-6 relative before:absolute before:left-2 before:top-2 before:bottom-2 before:w-px before:bg-border/50">
          {events.map((event, i) => (
            <div key={`${event.timestamp}-${i}`} className="relative pl-8 group">
              <div className={`absolute left-0 top-1.5 h-4 w-4 rounded-full bg-popover border-2 z-10 flex items-center justify-center ${i === events.length - 1 ? "border-emerald-500" : "border-primary"}`}>
                <div className={`h-1.5 w-1.5 rounded-full ${i === events.length - 1 ? "bg-emerald-500" : "bg-primary"}`} />
              </div>

...
export async function setup() {
  ...
  try {
    ...
    // Create mock auth schema to satisfy RLS policies using auth.uid()
    ...
    await pool.query(`
      ...
      $$ LANGUAGE plpgsql;
    `);
    console.log("[Integration Setup] Created mock 'auth.uid()' function.");
  } catch (err) {
    console.warn(
      "[Integration Setup] Failed to install extension / mock auth:",
      err
    );
  } finally {
    await pool.end();

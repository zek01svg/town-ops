...
def test_integration_accept_job_full_flow() -> None:
    ...
    ).mock(return_value=Response(200, json={"assignment": {"id": ASSIGNMENT_ID, "status": "accepted"}}))
    ...
    respx.post("http://localhost:5004/api/appointments").mock(
        return_value=Response(201, json={"appointment": {"id": appt_id, "status": "scheduled"}})
    )
...
def test_integration_cors_headers_present() -> None:
    ...
    respx.post("http://localhost:5004/api/appointments").mock(
        return_value=Response(201, json={"appointment": {"id": str(uuid4()), "status": "scheduled"}})
    )

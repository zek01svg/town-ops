...
class AtomClientError(Exception):
  """Exception raised for client errors in atom services."""

  def __init__(self, label: str, status_code: int, text: str) -> None:
    super().__init__(f"{label} failed with HTTP {status_code}: {text}")


def _raise_for_status(response: httpx.Response, label: str) -> None:
  """Raise a descriptive AtomClientError for non-2xx responses."""
  if response.is_error:
    raise AtomClientError(label, response.status_code, response.text)

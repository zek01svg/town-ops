...
setup("contractor login", async ({ page }) => {
  ...
  await page.getByLabel(/email/i).fill(process.env.CONTRACTOR_EMAIL ?? "aljunied@townops.dev");
  await page.getByLabel(/password/i).fill(process.env.CONTRACTOR_PASSWORD ?? "Contractor@123");

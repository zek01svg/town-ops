...
test.describe("Contractor Dashboard", () => {
  ...
  test("shows dashboard with contractor-specific stat cards", async ({ page }) => {
    await expect(page.getByText(/backlog/i).first()).toBeVisible();
    await expect(page.getByText(/dispatched/i).first()).toBeVisible();
  });

  test("dashboard only shows cases assigned to logged-in contractor", async ({ page }) => {
    // Cases section should be visible (even if empty — contractor may have no cases)
    await expect(
      page.locator(".kanban, [data-testid='kanban'], [class*='kanban']").first()
        .or(page.getByText(/no cases/i).first())
    ).toBeVisible({ timeout: 10_000 });
  });

  test("clicking a case card opens audit trail", async ({ page }) => {
    const cards = page.locator("[data-testid='case-card'], [class*='kanban-card'], .case-card");
    const count = await cards.count();
    test.skip(count === 0, "No cases assigned to this contractor — skipping audit trail test");
  ...
  test("Acknowledge Job button visible on pending assignment", async ({ page }) => {
    const acknowledgeBtn = page.getByRole("button", { name: /acknowledge/i });
    const count = await acknowledgeBtn.count();
    test.skip(count === 0, "No pending assignments — skipping acknowledge test");
  ...
  test("Close Job button opens the close job sheet", async ({ page }) => {
    ...
    test.skip(count === 0, "No accepted jobs to close — skipping close job test");

    await closeBtn.first().click();
    await expect(page.getByRole("dialog")).toBeVisible({ timeout: 5_000 });
    await expect(page.getByText(/before photo|completion report/i).first()).toBeVisible();
  });
  ...
  test("SLA countdown timer visible on pending assignment", async ({ page }) => {
